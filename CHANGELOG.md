...
- The core install depends only on networkx, numpy, and pydantic. pandas, scipy, and matplotlib moved to the `viz` and `bayes` extras.
...
- Packaging uses `pyproject.toml`, and the version is defined once in `ecoanalyst/_version.py`.
