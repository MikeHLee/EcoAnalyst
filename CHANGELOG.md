...
- A causal layer, `ecoanalyst.causal`: a `CausalModel` of variables with linear equations on an identity, logit, or log link, bound to node and edge parameters. `simulate()` and `compare()` estimate the effect of interventions (`do`) on flow outcomes with shared random draws; `fit()` updates the equations from data (normal-inverse-gamma posterior, or a Laplace approximation for binary variables, with priors scaled to the data so that results do not depend on units; `autoscale=False` gives fixed-scale priors); `adjustment_set()` and `is_valid_adjustment()` apply the back-door criterion; `to_gml()` exports the graph for DoWhy. `predict()` gives one-step predictions, `do` takes a value per sample, and `draw_parameters()` keeps each sample's coefficients fixed across repeated calls for step-by-step simulation. The model is saved in both JSON formats, and four MCP tools expose it. See `examples/causal_cold_chain.py`.
